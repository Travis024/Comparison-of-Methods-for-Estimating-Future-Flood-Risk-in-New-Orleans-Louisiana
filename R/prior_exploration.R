...
data.dir.priors <- data.dir
files.priors <- c("h750a_wilmington.csv", "h752a_ftpulaski.csv", "h753a_mayport.csv", 
                  "h759a_stpetersburg.csv", "h762a_pensacola.csv", "h769a_rockport.csv", 
                  "h775a_galveston.csv")
...
if(FALSE) {
  ...
  for (dd in 1:length(data.priors)) {
    ...
    for (mm in 1:nmodel) {
      ...
      # set up storm surge covariate
      n_data <- length(data.priors[[dd]][,"year"])
      first35 <- 1:35
      last35 <- (n_data-35+1):n_data
      if (mm > 1) {auxiliary <- trimmed_forcing(data.priors[[dd]][first35,"year"], time_forc, temperature_forc)$temperature
      } else {auxiliary <- NULL}
      # calibration
      out.deoptim <- DEoptim(neg_log_like_gev, lower=gev_models[[mm]]$bound_lower, upper=gev_models[[mm]]$bound_upper,
                             DEoptim.control(NP=NP.deoptim,itermax=niter.deoptim,F=F.deoptim,CR=CR.deoptim,trace=FALSE),
                             parnames=gev_models[[mm]]$parnames, data_calib=data.priors[[dd]][first35,"lsl_max"], auxiliary=auxiliary)
      deoptim.priors1[[mm]][dd,] <- out.deoptim$optim$bestmem
      colnames(deoptim.priors1[[mm]]) <- gev_models[[mm]]$parnames
      if (mm > 1) {auxiliary <- trimmed_forcing(data.priors[[dd]][last35,"year"], time_forc, temperature_forc)$temperature
      } else {auxiliary <- NULL}
      # calibration
      out.deoptim <- DEoptim(neg_log_like_gev, lower=gev_models[[mm]]$bound_lower, upper=gev_models[[mm]]$bound_upper,
                             DEoptim.control(NP=NP.deoptim,itermax=niter.deoptim,F=F.deoptim,CR=CR.deoptim,trace=FALSE),
                             parnames=gev_models[[mm]]$parnames, data_calib=data.priors[[dd]][last35,"lsl_max"], auxiliary=auxiliary)
      deoptim.priors2[[mm]][dd,] <- out.deoptim$optim$bestmem
      colnames(deoptim.priors2[[mm]]) <- gev_models[[mm]]$parnames
      tend <- proc.time()
      print(paste('... done. Took ',round(as.numeric(tend-tbeg)[3]/60,2),' minutes', sep=''))
    }
    tend0 <- proc.time()
